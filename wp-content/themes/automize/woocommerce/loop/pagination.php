	...
	if ( in_array($pagination_type, [ 'load_more', 'infinity' ], true) ) {
		...
		<div class="pagination_count mb-[15px] pt-6 text-xs">
			...
			echo sprintf(
				wp_kses(
				/* translators: %1$s: Number of products being shown. %2$s: Total number of products. */
					__('Showing %1$s of %2$s products', 'automize'),
		...
		if ( $current < $total ) {
			?>
			<button
				class="text-center h-12 px-5 rounded-[4px] min-w-[250px] rbb-load-more duration-300 text-[color:var(--rbb-general-heading-color)] hover:text-white bg-[#ebebeb] hover:bg-[color:var(--rbb-general-primary-color)] text-[10px] <?php echo esc_attr(( 'infinity' === $pagination_type ) ? 'hidden' : ''); ?>"
				data-url="<?php echo esc_url($url); ?>"><?php echo esc_html__('Load more items', 'automize'); ?>
