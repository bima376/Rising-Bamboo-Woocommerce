...
if ( ! wc_ship_to_billing_address_only() && wc_shipping_enabled() ) {
	$get_addresses = apply_filters(
		...
		[
			'billing'  => __('Billing address', 'automize'),
			'shipping' => __('Shipping address', 'automize'),
		],
		$customer_id
	);
} else {
...
<div class="rbb-account__address">
	<p class="rbb-account__address-top text-[0.8125rem] border-t-[3px] rounded-b-lg px-6 sm:px-7 py-4 mb-[30px]">
		<span class="icon-i mr-4 sm:mr-5 inline-block text-white rounded-full text-center">i</span><?php echo wp_kses_post(apply_filters('woocommerce_my_account_my_address_description', esc_html__('The following addresses will be used on the checkout page by default.', 'automize'))); ?>
	</p>
	<div class="grid <?php echo esc_attr(( ! wc_ship_to_billing_address_only() && wc_shipping_enabled() ) ? 'grid-cols-2' : 'grid-cols-1'); ?> gap-[30px] lg:gap-[30px] woocommerce-Addresses addresses">
		<?php
				...
				<div class="rbb-account__address-content border-2 rounded-2xl overflow-hidden">
					<header class="woocommerce-Address-title rbb-account__address-header font-bold px-[30px] py-[18px] flex items-center justify-between">
						...
						<div class="rbb-account__edit flex items-center">
							<a href="<?php echo esc_url(wc_get_endpoint_url('edit-address', $name)); ?>" class="edit leading-5">
								<?php 
								if ( $address ) {
									echo esc_html__('Edit', 'automize');
								} else {
									echo esc_html__('Add', 'automize');
								}
								?>
					...
					<p class="rbb-address__content bg-white rbb-address__billing rounded-2xl px-8 pt-8 pb-11 leading-[30px]">
						...
						if ( $address ) {
							...
							esc_html_e('You have not set up this type of address yet.', 'automize'); }
														/**
