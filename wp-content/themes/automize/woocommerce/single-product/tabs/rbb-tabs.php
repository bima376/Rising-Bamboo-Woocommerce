...
<?php if ( ! empty($rbb_tabs) ) { ?>
	<?php $tab_title = ( 1 !== count($rbb_tabs) ) ? 'tab-title' : ''; ?>
	<div class="pb-[85px]">
		<div class="tabs-product relative">
			<ul class="whitespace-nowrap title_heading overflow-x-auto md:overflow-x-visible flex lg:absolute relative top-0 left-0 z-10 <?php echo esc_attr($tab_title); ?>" id="tabs-nav">
				<?php
				foreach ( $rbb_tabs as $key => $rbb_tab ) {
					?>
					<li class="nav-item tab-a relative cursor-pointer uppercase text-2xl mr-[60px] transition ease-out duration-200" data-id="<?php echo esc_attr($key); ?>">
