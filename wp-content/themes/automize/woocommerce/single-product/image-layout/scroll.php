...
<div class="rbb-product-single-image-scroll col-span-1 <?php echo esc_attr(implode(' ', array_map('sanitize_html_class', $args['wrapper_classes']))); ?>">
	<div class="lg:flex <?php echo esc_attr(( 'left' === $thumbnail_position ) ? 'flex-row-reverse' : ''); ?>">
		<div id="rbb-gallery-lightbox" class="overflow-hidden">
			<div class="rbb-product-gallery" >
				...
				foreach ( $args['attachment_ids'] as $attachment_id ) {
					...
					?>
					<div class="rbb-product-gallery__image item rounded-lg overflow-hidden mb-[15px] <?php echo esc_attr(( 1 === $i ) ? 'act' : ''); ?>" data-position="<?php echo esc_attr($i); ?>">
		...
		<div class="rbb-product-thumbs -mx-[7px] lg:w-[105px] flex-none -my-[7.5px] lg:pt-0 pt-[8px] lg:mx-0
			<?php echo esc_attr(( 'left' === $thumbnail_position ) ? 'lg:mr-[15px]' : ''); ?>
			<?php echo esc_attr(( 'right' === $thumbnail_position ) ? 'lg:ml-[15px]' : ''); ?>
		">
			...
			>
				...
				foreach ( $args['attachment_ids'] as $attachment_id ) {
					$attachment_props = wc_get_product_attachment_props($attachment_id, $post);
					?>
					<div class="item <?php echo esc_attr(( 1 === $j ) ? 'active' : ''); ?> <?php echo esc_attr(( 'left' === $thumbnail_position || 'right' === $thumbnail_position ) ? 'lg:py-[7.5px]  lg:px-0 px-[7.5px]' : 'px-4'); ?>" data-position="<?php echo esc_attr($j); ?>">
