		...
		if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key) ) {
			...
			<li class="woocommerce-mini-cart-item mb-2.5 relative flex items-center rounded duration-300 <?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?>">
				...
				<div class="product-information pr-10">
					<a class="product-name font-semibold pt-0.5 mb-[5px] block" href="<?php echo esc_url($product_permalink); ?>">
						<span class="line-clamp-2"><?php echo wp_kses_post($product_name); ?></span>
					</a>
					...
					<div class="flex text-xs leading-6">
						...
						echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							...
							sprintf(
								...
								esc_attr__('Remove this item', 'automize'),
								esc_attr($product_id),
...
<?php else : ?>
	<div class="pt-[42px] px-5 text-center">
		...
		<p class="title text-base pt-11 pb-[3px]"><?php esc_html_e('No products in the cart.', 'automize'); ?></p>
		<p class="woocommerce-mini-cart__empty-message max-w-[280px] inline-block">
			<?php
			esc_html_e('Your cart is currently empty. Let us help you find the perfect item!', 'automize');
			?>
		</p>
