...
if ( isset($args['overlay']) && true === $args['overlay'] ) {
	...
	?>
<form role="search" method="get" class="<?php echo esc_attr($class_form); ?>" action="<?php echo esc_url(home_url('/')); ?>" data-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" data-noresult="<?php echo esc_attr__('No Result', 'automize'); ?>" data-limit="<?php echo esc_attr($limit); ?>">
	<div class="relative input-group z-20 flex bg-white rounded-[30px]">
			...
			<div class="rbb-search-categories relative flex-grow h-12">
				<div class="current-category flex items-center h-full w-full inline-block py-2 px-5 cursor-pointer leading-[48px]" onclick="RbbThemeSearch.openSearchCategories(event)">
					...
					if ( isset($categories[ $category_slug ]) ) {
						...
						echo esc_html__('All Categories', 'automize');
					}
					?>
					</span>
					<i class="rbb-icon"></i>
				...
				<ul class="categories absolute left-0 top-full z-50 py-2 bg-white rounded-[3px] shadow-lg">
					<li class="dropdown-item cursor-pointer font-bold text-[11px] px-5 py-1.5 active"
						onclick="RbbThemeSearch.chooseCategory(event, '')"><?php echo esc_html__('All Categories', 'automize'); ?></li>
		...
		<?php } ?>
		<input class="input-search s w-full pb-0 border-solid" type="text" value="<?php echo get_search_query(); ?>" name="s" placeholder="<?php echo esc_attr__('What Are You Looking For ?', 'automize'); ?>" autocomplete="off"/>
		<button id="search" class="button-icon btn-search duration-300 absolute w-[50px] h-[50px] text-center -right-[1px] -top-[1px]"
				type="submit">
			<span class="search-icon <?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_ICON)); ?> text-xl"></span>
		</button>
		<div class="hidden btn-search_clear-text absolute right-0 top-2.5 w-10 h-10 leading-10 text-center text-[#cdcdcd]"><i class="rbb-icon-close-1"></i></div>
	</div>
...
<form role="search" method="get" class="<?php echo esc_attr($class_form); ?>" action="<?php echo esc_url(home_url('/')); ?>" data-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" data-noresult="<?php echo esc_attr__('No Result', 'automize'); ?>" data-limit="<?php echo esc_attr($limit); ?>">
	<div class="relative input-group flex bg-white rounded-[--rbb-search-input-border-radius]">
			...
			<div class="inline-table items-center search-categories">
				<div class="rbb-search-categories border-r border-r-[#edecec] flex h-[50px]">
					...
					<ul class="categories absolute left-0 top-full z-50 py-2 bg-white rounded-[3px] shadow-lg">
						...
						onclick="RbbThemeSearch.chooseCategory(event, '')"><?php echo esc_html__('Home', 'automize'); ?></li>
						<?php foreach ( $categories as $cat_slug => $cat_name ) { ?>
							<li class="dropdown-item cursor-pointer text-[11px] px-5 py-1.5 whitespace-nowrap <?php echo esc_attr(( $cat_slug === $category_slug ) ? 'active' : ''); ?>"
								onclick="RbbThemeSearch.chooseCategory(event,'<?php echo esc_attr($cat_slug); ?>')">
		...
		<div class="relative w-full">
			<input class="input-search s w-full pb-0 border-solid" type="text" placeholder="<?php echo esc_attr__('Enter Your Keyword', 'automize'); ?>" value="<?php echo get_search_query(); ?>" name="s" autocomplete="off"/>
			<label class="whitespace-nowrap md:block hidden duration-300 pointer-events-none text-[10px] text-[#a1a1a1] uppercase absolute top-[51%] -translate-y-1/2 left-[30px]"><?php echo esc_html__('Enter Your Keyword', 'automize'); ?></label>
			<button class="button-icon btn-search absolute flex items-center w-11 h-11 text-center md:right-[3px] right-0 top-[3px] duration-300" type="submit">
				...
				<span class="search text-[0px]"><?php echo esc_html__('search', 'automize'); ?></span>
			</button>
