...
<div id="<?php echo esc_attr($id); ?>" class="relative group overflow-hidden bg-white rounded-[10px] shadow-[11px_12px_20px_0px_rgba(0,0,0,0.05)] <?php echo esc_attr($layout); ?>">
	<div class="flex relative items-center justify-center">
		<div class="lg:w-[53%] w-1/2 px-4 py-2">
			<div class="<?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> relative text-center justify-center flex items-centert overflow-hidden">
				<?php
				$img_count = 0;
				foreach ( $images as $image ) {
					...
					?>
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img group-hover:animate-[hexagonIcon_1s_ease-in-out] duration-300">
							<?php
						...
						if ( $image_link_attr_string ) {
							?>
						<a <?php echo esc_attr($image_link_attr_string); ?> >
						<?php } ?>
		...
		<div class="lg:w-[47%] w-1/2 md:p-[15px] p-2.5">
			<div class="text-white">
				<div class="title_block">
					<h4 class="title text-base"><?php echo wp_kses_post($title); ?></h4>
					<p class="sub_title"><?php echo wp_kses_post($sub_title); ?></p>
				</div>
				<div class="desc text-xs  mt-1"><?php echo wp_kses_post($description); ?></div>
			</div>
			<?php
			...
			<div class="mt-2 text-[12px]">
				...
				foreach ( $custom_html as $html ) {
					...
					echo "<div class='custom-html text-left " . esc_attr($repeater_custom_html_class) . ' ' . esc_attr($position) . "'>" . wp_kses_post($html[ $widget->get_name_setting('custom_html') ]) . '</div>';
				}
				?>
			</div>
			<div class="flex items-left justify-left mt-2">
				<?php
				foreach ( $buttons as $button ) {
					...
					<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
						...
						<?php echo ! empty($button_url_nofollow) ? 'rel="' . esc_attr($button_url_nofollow) . '"' : ''; ?>
							class="inline-block btn text-left pl-6 pr-[7px] group rbb-button relative duration-300 <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
						<?php
					...
					>
						...
						<?php } ?>
						<span class="button-text !capitalize duration-300 inline-block"><?php echo wp_kses_post($button_name); ?></span>
						<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
							<span class="button-text">
									...
									if ( ! empty($button_icon['value']) ) {
										...
										?>
										<i class="rbb-icon-direction-711 pl-2 text-xl inline-block align-sub" aria-hidden="true"></i>
									<?php } ?>
						</span>
						<?php } ?>
					</a>
