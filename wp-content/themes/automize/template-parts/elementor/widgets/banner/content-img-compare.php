...
<div id="<?php echo esc_attr($id); ?>" class="banner-default min-h-[526px] relative overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="2xl:container mx-auto">
		<div class="xl:grid xl:grid-cols-2 gap-4 ">
			<div class="xl:col-span">
				<div class="md:text-left text-center">
					<div class="title_block pb-2">
						<p class="sub_title text-lg text-[color:var(--rbb-general-secondary-color)]"><?php echo wp_kses_post($sub_title); ?></p>
						<?php
						if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
							...
							<div class="pb-5 banner-countdown -mx-[5px]">
								<div class="rbb-countdown flex justify-self-start <?php echo esc_attr($countdown_position); ?>" data-countdown-date="<?php echo esc_attr($countdown); ?>">
									...
									<div class="item-time"><span class="data-number">%S</span><span class="name-time"><?php echo esc_html__('Sec', 'automize'); ?></span></div>
								</div>
							</div>
							<?php
						}
						?>
						<h4 class="title xl:text-4xl md:text-3xl text-2xl"><?php echo wp_kses_post($title); ?></h4>
					</div>
					<div class="desc mt-4 max-w-[575px] text-white"><?php echo wp_kses_post($description); ?></div>
				</div>
			...
			<div class="xl:col-span-1 xl:pt-0 pt-5">
				<div class="rbb-before-after init">
					<div class="rbb-ba-wrap" data-filter="0">
						<?php
						$img_count      = 0;
						$reversed_array = array_reverse(array_slice($images, 0, 2));
						foreach ( $reversed_array as $image ) {
							...
							if ( 1 === $img_count ) { 
								?>
								<div class="before item" style="background-image: url('<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>')">
								</div>
								<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="rbb-ba-before <?php echo esc_attr($css_class); ?> <?php echo esc_attr($position); ?> <?php echo esc_attr($repeater_image_class); ?>"
								>
							<?php } else { ?>
								<div class="after item" style="background-image: url('<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>')">
								</div>
							<?php } ?>
						<?php ++$img_count; ?><?php } ?>
				...
				<div class="rbb-text-ba flex justify-between mt-3 font-bold uppercase text-2xl">
					<div class="rbb-before-header rbb-ba-text"><?php echo esc_html__('Before', 'automize'); ?></div>
					<div class="rbb-after-header rbb-ba-text"><?php echo esc_html__('After', 'automize'); ?></div>
				</div>
			</div>
		</div>
	</div>
