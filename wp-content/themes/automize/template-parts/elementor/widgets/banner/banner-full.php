...
<div id="<?php echo esc_attr($id); ?>" class="overflow-hidden <?php echo esc_attr($layout); ?>">
	<div class="relative">
		<div class="container mx-auto">
			<div class="max-w-[500px]">
				<div class="title_block max-w-[323px]">
					<p class="sub_title text-ms text-black"><?php echo wp_kses_post($sub_title); ?></p>
					<h4 class="title text-2xl"><?php echo wp_kses_post($title); ?></h4>
				</div>
				<div class="desc text-base mt-1"><?php echo wp_kses_post($description); ?></div>
			</div>
			<?php
			...
			?>
			<div class="flex items-left justify-left mt-[55px]">
				<?php
				foreach ( $buttons as $button ) {
					...
					<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
						...
						<?php echo ! empty($button_url_nofollow) ? 'rel="' . esc_attr($button_url_nofollow) . '"' : ''; ?>
							class="inline-block btn h-[50px] min-w-[140px] text-center leading-[50px] rounded-[50px] rbb-button relative px-[36px] <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
						<?php
