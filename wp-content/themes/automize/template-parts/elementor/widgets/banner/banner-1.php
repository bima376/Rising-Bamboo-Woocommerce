...
<div id="<?php echo esc_attr($id); ?>" class="relative overflow-hidden sm:rounded-[10px] <?php echo esc_attr($layout); ?>">
	<div class="lg:grid relative">
		<div class="mx-auto md:max-w-[460px] absolute left-0 z-10 text-left top-[50%] -translate-y-1/2 xl:px-[60px] lg:px-10 px-[15px]">
			<div class="text-white">
				<div class="title_block">
					<h4 class="title lg:text-4xl text-2xl"><?php echo wp_kses_post($title); ?></h4>
					<p class="sub_title lg:text-4xl text-2xl"><?php echo wp_kses_post($sub_title); ?></p>
				</div>
				<div class="desc mt-1"><?php echo wp_kses_post($description); ?></div>
			</div>
			<?php
			...
			?>
			<div class="flex items-left justify-left md:mt-6">
				<?php
				foreach ( $buttons as $button ) {
					...
					<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
						...
						<?php echo ! empty($button_url_nofollow) ? 'rel="' . esc_attr($button_url_nofollow) . '"' : ''; ?>
							class="inline-block btn sm:h-[50px] h-10 min-w-[140px] text-center sm:leading-[50px] leading-[40px] pl-6 pr-[7px] sm:rounded-[50px] rounded-[40px] rbb-button relative px-[36px] <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
						<?php
					...
					>
						...
						<?php } ?>
						<span class="button-text inline-block"><?php echo wp_kses_post($button_name); ?>
						<?php if ( $button_icon_status && 'after' === $button_icon_position ) { ?>
									...
									if ( ! empty($button_icon['value']) ) {
										...
										?>
										<i class="rbb-icon-direction-711 pl-2 text-xl align-sub" aria-hidden="true"></i>
									<?php } ?>
						<?php } ?>
						</span>
					</a>
		...
		<div class="col-span-12 lg:col-span-12">
			<div class="<?php echo esc_attr(( $parallax ) ? 'parallax' : ''); ?> relative text-center">
				...
				foreach ( $images as $image ) {
					...
					?>
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img min-h-[220px]">
							<?php
							if ( $image_link_attr_string ) {
								?>
							<a <?php echo wp_kses($image_link_attr_string, 'rbb-kses'); ?> >
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="<?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block w-full object-cover min-h-[220px] <?php echo esc_attr($css_class); ?> <?php echo esc_attr($position); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
