				...
				foreach ( $buttons as $button ) {
					...
					<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-' . esc_attr($button['_id']) . '\', event)"' : ''; ?>
						...
						<?php echo ! empty($button_url_nofollow) ? 'rel="' . esc_attr($button_url_nofollow) . '"' : ''; ?>
							class="inline-block btn h-[50px] min-w-[140px] text-center leading-[50px] pl-6 pr-[7px] rounded-[50px] rbb-button relative px-[36px] <?php echo esc_attr($button_position); ?> <?php echo esc_attr($button_class); ?>"
						<?php
				...
				foreach ( $images as $image ) {
					...
					<?php if ( 0 === $img_count ) { ?>
						<div class="banner-img group-hover:scale-105 duration-300">
							...
							<?php } ?>
							<img alt="Image" src="<?php echo esc_url($image[ $widget->get_name_setting('image') ]['url']); ?>" class="<?php echo esc_attr(( $parallax ) ? 'layer' : ''); ?> inline-block w-full min-h-[300px] object-cover <?php echo esc_attr($css_class); ?> <?php echo esc_attr($position); ?> <?php echo esc_attr($repeater_image_class); ?>"
							<?php if ( $parallax ) { ?>
