...
if ( count($data['products']) ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb_woo_products overflow-hidden <?php echo esc_attr($layout); ?>">
		...
		<?php } ?>
		<div class="max-w-[1580px] pb-3 mx-auto overflow-hidden">
			<div class="text-center">
				<div class="title_left">
					<div class="mb-7">
						<?php if ( $show_title ) { ?>
							<span class="title block text-2xl pb-1.5"><?php echo wp_kses_post($data['title']); ?></span>
		...
		<div class="rbb-product-list grid gap-0 xl:grid-cols-3 grid-cols-2">
			...
			foreach ( $data['products'] as $product_id => $product ) { 
				if ( $product_id <= 8 ) {
					if ( 0 === $product_id || 4 === $product_id || 5 === $product_id ) { 
						?>
						<div class="
						<?php 
						if ( 4 === $product_id ) {
	...
	col-span-1 block_right xl:order-3<?php } ?>">
						<?php } ?>
						<?php if ( 0 === $product_id || 5 === $product_id ) { ?>
							<div class="grid lg:grid-cols-2 grid-cols-1">
							<?php } ?>	
							<?php if ( 4 === $product_id ) { ?>
								<?php
								View::instance()->load(
									'elementor/widgets/woo-products/fragments/item-6',
									[
										'product' => $product,
									]
								);
							...
							<?php } else { ?>
								...
								?>
							<?php } ?>
							<?php if ( 3 === $product_id || 8 === $product_id ) { ?>
							</div>
						<?php } ?>
						<?php if ( 3 === $product_id || 4 === $product_id || 8 === $product_id ) { ?>
						</div>
					<?php } ?>
				<?php } ?>
			<?php } ?>
