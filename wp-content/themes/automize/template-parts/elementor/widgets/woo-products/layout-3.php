...
if ( count($data['products']) ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb_woo_products overflow-hidden <?php echo esc_attr($layout); ?>">
		...
		<div class="2xl:container px-[15px] mx-auto wow fadeInUp" data-wow-delay="0.1s">
			<div class="title_left text-center mb-5">
				<?php if ( $show_title ) { ?>
				<div class="border-title relative mb-8">
					<h2 class="title uppercase whitespace-nowrap text-2xl"><?php echo wp_kses_post($data['title']); ?></h2>
			...
			<div class="rbb-slick-carousel slick-carousel slick-carousel-center item-<?php echo esc_attr($slides_per_row); ?>" data-slick='{
			...
			<div class="all-products text-center pt-8 font-bold"><a class="h-[50px] px-12 leading-[50px] inline-block rounded-[50px] duration-300 text-black hover:text-white bg-[color:var(--rbb-general-button-bg-color)] hover:bg-[color:var(--rbb-general-secondary-color)]" href="<?php echo esc_url(get_permalink(wc_get_page_id('shop'))); ?>"><?php echo esc_html__('All Products', 'automize'); ?></a></div>
		</div>
	</div>
	<?php
}
