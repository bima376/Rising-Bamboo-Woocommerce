...
?>
<div class="item">
	<div class="bg-white overflow-hidden rounded-[10px] border border-[#ffe200]">
		<div class="item-product relative bg-white px-2">
			<?php
			...
			if ( $show_countdown && $countdown_date_to ) {
				...
				if ( ( $current_date >= $countdown_date_start ) && ( $current_date <= $countdown_date_to ) ) {
					?>
					<div class="item-countdown !opacity-[1] duration-300 my-5">
		...
		<div class="product_info 2xl:px-5 px-4 2xl:mt-[37px] xl:mt-[50px] mt-[37px] pb-9">
			<div class="product-title">
				<a href="<?php echo esc_url($product->get_permalink()); ?>" class="product_name leading-6 block text-sm !font-semibold mb-3.5">
					<span class="line-clamp-2 min-h-[48px]"><?php echo wp_kses_post($product->get_name()); ?></span>
				</a>
			</div>
			<?php if ( $show_rating ) { ?>
				<div class="product_ratting text-amber-400 flex items-center mb-5">
			...
			<div class="product_price text-base font-bold">
				...
				if ( $show_percentage_discount && $product->get_sale_price() ) {
					...
					?>
					<label class="!inline-block align-top relative ml-[15px] min-w-[42px] text-white p-1 text-[0.625rem] rounded-[3px] flex leading-4 items-center h-[22px] justify-center bg-[color:var(--rbb-general-secondary-color)]">
						<?php echo '-' . esc_html($sale_percentage) . '%'; ?>
					</label>
				<?php } ?>
			</div>
			<div class="product-popup bg-white absolute inset-x-0 bottom-0 opacity-0 px-2">
			...
			<?php } ?>
				<div class="product_button flex mt-4 mb-5">
					<?php
					if ( $show_add_to_cart ) {
						$args      = [];
						$text_cart = '';
