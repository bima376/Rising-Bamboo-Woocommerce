...
if ( count($data['products']) ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb_woo_products <?php echo esc_attr($layout); ?>">
		...
		<div class="2xl:container px-[15px] mx-auto wow fadeInUp" data-wow-delay="0.1s">
			<div class="md:overflow-hidden mb-0.5 border-b-2 border-b-[color:var(--rbb-general-secondary-color)]">
				<div class="title_left flex block items-center justify-between">
					<?php if ( $show_title ) { ?>
				...
				<?php } ?>
					...
					?>
					<div class="flex items-center font-semibold text-[10px]"><i class="rbb-icon-menu-11 font-bold text-[color:var(--rbb-general-secondary-color)] mr-2.5 text-sm"></i><a class="duration-300 text-[#D7D7D7] hover:color-[color:var(--rbb-general-secondary-color)] uppercase" href="<?php echo esc_url(get_permalink(wc_get_page_id('shop'))); ?>"><?php echo esc_html__('View all products', 'automize'); ?></a></div>
				</div>
			</div>
