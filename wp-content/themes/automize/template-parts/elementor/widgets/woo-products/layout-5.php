...
if ( count($data['products']) ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb_woo_products <?php echo esc_attr($layout); ?>">
		...
		<div class="container mx-auto">
			<div class="text-left">
				<div class="title_left items-center overflow-hidden">
					<?php if ( $show_title ) { ?>
					<div class="flex justify-between items-center mb-4">
						<div class="text-white">
							<h2 class="title text-2xl"><?php echo wp_kses_post($data['title']); ?></h2>
							<span class="sub-title block"><?php echo wp_kses_post($subtitle); ?></span>
						</div>
						<div class="all-products text-center font-bold"><a class="h-[33px] px-5 leading-[33px] inline-block rounded-[33px] duration-300 text-black hover:text-white bg-[color:var(--rbb-general-button-bg-color)] hover:bg-[color:var(--rbb-general-secondary-color)]" href="<?php echo esc_url(get_permalink(wc_get_page_id('shop'))); ?>"><?php echo esc_html__('Shop All', 'automize'); ?></a></div>
					</div>
				<?php } ?>
					<?php
