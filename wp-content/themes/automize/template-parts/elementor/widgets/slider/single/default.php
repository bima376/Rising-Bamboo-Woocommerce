...
if ( $sliders ) {
	?>
	<div class="rbb-elementor-slider rounded-[10px] overflow-hidden <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		<div class="flex one-item rbb-slick-carousel slick-carousel slick-carousel-center" data-slick='{
			...
			foreach ( $sliders as $slider ) {
				...
				<div class="w-full item elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?>">
					<div class="item-content relative">
						<div class="md:absolute md:py-0 top-0 w-full h-full">
								...
								if ( $slider_title || $slider_sub_title || $description || $button_1 || $button_2 ) {
									...
									switch ( $horizontal_align ) {
										...
										case 'center':
											...
											break;
										default:
											$horizontal_align_class = 'left-0 md:text-left text-left';
									}
									...
									switch ( $vertical_align ) {
										case 'bottom':
											...
											break;
										case 'middle':
											$vertical_align_class = 'top-1/2 md:-translate-y-1/2 translate-y-1/2';
											break;
										default:
										...
										<div class="info-inner md:pl-10">
											...
											if ( ! empty($countdown) && ( time() < strtotime($countdown) ) ) {
												?>
													<div class="rbb-countdown wow fadeInLeft flex justify-self-end mb-14 <?php echo esc_attr($countdown_position); ?>" data-visibility-start ="invisible" data-countdown-date="<?php echo esc_attr($countdown); ?>">
														<div class="item-time days"><span class="data-number">%D</span><span class="name-time"><?php echo esc_html__('Day%!H', 'automize'); ?></span></div>
											...
											if ( $slider_sub_title ) {
												...
												<h3 data-visibility-start="<?php echo esc_attr($subtitle_visibility); ?>"
													...
													class="sub-title text-sm mb-1.5 <?php echo esc_attr($subtitle_visibility); ?>"><?php echo wp_kses_post($slider_sub_title); ?></h3>
											<?php } ?>
											<?php 
											if ( $slider_title ) {
											...
											if ( $description ) {
												...
												<div data-visibility-start="<?php echo esc_attr($description_visibility); ?>"
													...
													<?php echo ! empty($description_animation_delay) ? 'data-animation-delay="' . esc_attr($description_animation_delay) . '"' : ''; ?>class="description md:truncate md:mb-4 lg:whitespace-normal lg:mb-6 <?php echo esc_attr($description_visibility); ?>"><?php echo wp_kses_post($description); ?></div>
											<?php } ?>
											<?php
											$horizontal_align_center = $slider[ $slick->get_name_setting('horizontal_align') ] ?? 'center';

											?>
											<div class="button-group flex md:justify-start justify-left items-center text-left <?php echo ( 'center' !== $horizontal_align_center ) ? '' : 'justify-center'; ?>">
												<?php
												if ( $button_1 ) {
													...
													<a <?php echo ( 'url' !== $button_1_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-01-' . esc_attr($slider['_id']) . '\', event)"' : ''; ?>
														...
														class="rbb-slick-button duration-300 button-1 h-[50px] leading-[48px] px-7 min-w-[140px]  rounded-full inline-block text-center <?php echo esc_attr($button_1_visibility); ?>"
														<?php
														...
														?>
													>
														<span class="button-text inline-block align-middle font-bold text-xs"><?php echo wp_kses_post($button_1); ?></span>
														<span class="button-icon hidden align-middle ml-4 w-10 h-10 leading-10 rounded-full">
														<?php 
														if ( $button_1_icon ) {
													...
													if ( 'url' !== $button_1_type ) {
														...
														<div
																...
																class="<?php echo esc_attr($class_string); ?>" data-modal-animation="<?php echo esc_attr($modal_effect); ?>">
																...
																<div class="rbb-modal-body">
																	...
																	if ( 'video' === $button_1_type ) {
																		...
																		<video width="640" height="360" controls>
																			...
																			<?php echo esc_html__('Your browser does not support the video tag.', 'automize'); ?>
																		</video>
																		<?php 
												...
												if ( $button_2 ) {
													...
													<a <?php echo ( 'url' !== $button_2_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-button-02-' . esc_attr($slider['_id']) . '\', event)"' : ''; ?>
															...
															?>
													>
													<span class="button-icon hidden xl:w-14 xl:h-14 w-12 h-12 rounded-full text-center inline-block align-middle">
								...
								for ( $i = 1; $i <= $slick->images_per_slider; $i++ ) {
									...
									if ( $slider[ $slick->get_name_setting('image_' . $i) ] && $slider[ $slick->get_name_setting('image_' . $i) ]['url'] ) {
										...
										<img
												...
												alt="<?php echo esc_attr__('Image Slider ', 'automize') . esc_attr($i); ?>"
										>
										<?php
									}
								}
