			...
			<div class="block_title text-center flex justify-center items-center">
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					...
					<div class="block_slide">
						<div class="rbb-slick-carousel slick-carousel flex slick-carousel-center load-item-<?php echo esc_attr($sliders_to_show_default); ?>" data-slick='{
						...
						foreach ( $sliders as $slider ) {
							...
							?>
							<div class="item elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?> basis-0 max-w-full">
								<div class="block__content relative group duration-300 md:flex items-center justify-center text-center">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<a title="brand" <?php echo ! empty($content_link_attr_string) ? $content_link_attr_string : 'href="' . esc_url($button_url) . '"'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
										<img class="max-w-[120px] max-h-[50px] opacity-50 duration-300 group-hover:opacity-100 inline-block group-hover:animate-[hexagonIcon_1s_ease-in-out]" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
									</a>
									<?php } ?>
									<h4 class="md:text-[11px] text-[10px] leading-5 uppercase font-semibold text-white pr-2.5 flex justify-center items-center">
										<?php
										if ( $content_link_attr_string ) {
											?>
											<a class="text-white" <?php echo ! empty($content_link_attr_string) ? $content_link_attr_string : 'href="' . esc_url($button_url) . '"'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
												<?php echo wp_kses_post($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>
											</a>
											<?php
										} else {
											echo wp_kses_post($slider[ $slick->get_name_setting('multiple_content_title') ]);
										}
										?>
									</h4>
									<?php if ( ! empty($slider[ $slick->get_name_setting('multiple_content_description') ]) ) { ?>
									<p class="text-center"><?php echo esc_html($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></p>
								<?php } ?>
									<?php
									if ( $button ) {
										...
										class="rbb-slick-button duration-300 btn_view-all inline-flex">
										<span class="md:text-[11px] text-[10px] font-bold inline-flex justify-center items-center text-[color:var(--rbb-general-secondary-color)]">
