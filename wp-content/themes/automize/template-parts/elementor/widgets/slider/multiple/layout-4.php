							...
							<div class="item elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?> p-2.5">
								<div class="block__content flex items-center relative duration-300 bg-[#f3f3f3] hover:bg-white text-center rounded-[10px] md:p-[15px] px-2.5 md:min-h-[90px] min-h-[80px] hover:shadow-[6px_6px_6px_1px_rgba(0,0,0,0.08)] border border-[#f3f3f3] hover:border-[#ffe200]">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<img class="!max-w-[46px] md:w-[46px] w-10 !inline-block" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
										<?php
									}
									?>
