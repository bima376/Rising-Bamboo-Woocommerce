...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<div class="2xl:container px-[15px] mx-auto">
			<div class="block_title text-center flex justify-between items-center mb-9">
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					<div class="title_block text-left inline-block">
						<?php
						if ( $widget_sub_title ) {
							?>
							<p class="sub-title pb-3"><?php echo esc_html($widget_sub_title); ?></p> <?php } ?>
							<?php
							if ( $widget_title ) {
								?>
								<h4 class="main-title text-2xl uppercase"><?php echo esc_html($widget_title); ?></h4>
							<?php } ?>
							</div>
						<?php } ?>
						...
						<div class="flex items-center">
							<div class="slick-arrow">
								<span class="prev_custom w-9 h-9 rounded-full flex items-center justify-center cursor-pointer">
								</span>
							</div>
							<i class="rbb-icon-view-grid-1 text-black text-base"></i>					
							<div class="slick-arrow">
								<span class="next_custom w-9 h-9 rounded-full flex items-center justify-center cursor-pointer"></span>
							</div>
						</div>
						<?php } ?>
					</div>
					<div class="block_slide overflow-hidden">
						<div class="rbb-slick-carousel slick-carousel flex slick-carousel-center item-row load-item-<?php echo esc_attr($sliders_to_show_default); ?>" data-slick='{
							"arrows": <?php echo esc_attr($show_arrows); ?>,
							"appendArrows": "#<?php echo esc_attr($id); ?> .slick-arrow",
							"prevArrow": "#<?php echo esc_attr($id); ?> .slick-arrow .prev_custom",
							"nextArrow": "#<?php echo esc_attr($id); ?> .slick-arrow .next_custom",
						...
						foreach ( $sliders as $slider ) {
							...
							?>
							<div class="item elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?> basis-0 max-w-full mb-5">
								<div class="block__content flex items-center relative group duration-300 bg-white text-left rounded-[10px] p-[15px] shadow-[11px_12px_20px_0px_rgba(0,0,0,0.05)]">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<div class="lg:w-3/5 w-1/2 text-center mr-[15px] overflow-hidden">
										<img class="!max-h-[250px] !inline-block group-hover:animate-[hexagonIcon_1s_ease-in-out]" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
									</div>
									<?php } ?>
									<div class="lg:w-2/5 w-1/2">
									<h4 class="text-base pb-4 uppercase">
										<?php
									...
									<?php if ( ! empty($slider[ $slick->get_name_setting('multiple_content_description') ]) ) { ?>
										<div class="pb-3"><?php echo wp_kses_post($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></div>
									<?php } ?>
									...
									if ( $button ) {
										...
										?>
										<a <?php echo ( 'url' !== $button_type ) ? 'onclick="RisingBambooModal.modal(\'#rbb-modal-multiple-button-' . esc_attr($slider['_id']) . '\', event)"' : ''; ?>
										href="<?php echo esc_url($button_url); ?>" <?php echo esc_attr($button_url_target); ?> <?php echo esc_attr($button_url_nofollow); ?>
										class="rbb-slick-button btn_view-all inline-flex text-xs -ml-[15px]">
										<span class="rounded-full duration-300 h-[30px] leading-[30px] px-[15px] inline-flex text-[#CDCDCD] hover:text-[#222] hover:bg-[color:var(--rbb-general-button-bg-color)]">
											...
											if ( $button_icon ) {
												...
												?>
											<?php } else { ?>
												<i class="rbb-icon-direction-711 pl-2 text-xl inline-block align-sub"></i>
											<?php } ?>
										</span>
									</a>
										<?php
										if ( 'url' !== $button_type ) {
											?>
										<div id="rbb-modal-multiple-button-<?php echo esc_attr($slider['_id']); ?>" class="<?php echo esc_attr($class_string); ?>" data-modal-animation="<?php echo esc_attr($modal_effect); ?>">
