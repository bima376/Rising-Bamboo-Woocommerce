...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<?php } ?>
		<div class="2xl:container mx-[15px]">
			<div class="block_title text-center flex justify-center items-center">
				<?php if ( 'true' === $show_arrows ) { ?>
					<div class="slick-arrow-custom text-lg text-white">
						<span class="prev_custom mr-4 w-[40px] h-[40px] rounded-full bg-white text-black flex items-center justify-center cursor-pointer"><i class="rbb-icon-direction-36"></i>
						</span>
					</div>
				<?php } ?>
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					<div class="title_block inline-block mb-8">
						<?php
						if ( $widget_sub_title ) {
							?>
						...
						<?php if ( 'true' === $show_arrows ) { ?>
							<div class="slick-arrow-custom text-lg text-white">
								<span class="next_custom ml-4 w-[40px] h-[40px] rounded-full bg-white text-black flex items-center justify-center cursor-pointer"><i class="rbb-icon-direction-39"></i></span>
							</div>
						<?php } ?>
					</div>
					<div class="block_slide">
						<div class="rbb-slick-carousel slick-carousel flex slick-carousel-center item-row grid-rows-2 load-item-<?php echo esc_attr($sliders_to_show_default); ?>" data-slick='{
						...
						foreach ( $sliders as $slider ) {
							...
							<div class="item elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?> basis-0 max-w-full mb-5">
								<div class="block__content relative group duration-300 border border-[#FFFFFF1A] bg-[#091017] text-center rounded-[10px] md:px-5 px-2 py-[18px] hover:shadow-[7px_7px_15px_rgba(0,0,0,0.6)]">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<div class="flex justify-center pt-6 pb-8">
										<img class="max-w-[70px] max-h-[70px] !inline-block group-hover:animate-[hexagonIcon_1s_ease-in-out]" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
									</div>
									<?php } ?>
									<h4 class="text-base pb-0.5">
										<?php
									...
									<?php if ( ! empty($slider[ $slick->get_name_setting('multiple_content_description') ]) ) { ?>
										<div class="text-center pb-3"><?php echo wp_kses_post($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></div>
									<?php } ?>
									...
									if ( $button ) {
										...
										class="rbb-slick-button w-[62px] duration-300 h-[62px] absolute bottom-0 opacity-0 left-1/2 translate-x-[-50%] translate-y-[50%] btn_view-all rounded-full inline-flex justify-center items-center">
										<span class="rounded-full w-[50px] h-[50px] text-[15px] text-white inline-flex justify-center items-center bg-[color:var(--rbb-general-secondary-color)] hover:bg-[color:var(--rbb-general-primary-color)]">
											<?php
											if ( $button_icon ) {
