...
if ( $sliders ) {
	...
	<div id="<?php echo esc_attr($id); ?>" class="rbb-elementor-slider <?php echo esc_attr($layout_type); ?> <?php echo esc_attr($layout); ?>">
		...
		<?php } ?>
		<div class="2xl:container 2xl:mx-auto mx-[15px]">
			<div class="block_title text-center flex justify-center items-center">
				...
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					...
					<div class="block_slide">
						<div class="rbb-slick-carousel slick-carousel flex slick-carousel-center item-row grid-rows-2" data-slick='{
						...
						foreach ( $sliders as $slider ) {
							...
							?>
							<div class="item elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?> basis-0 grow max-w-full">
								<div class="block__content relative group duration-300 bg-white flex items-center justify-center min-h-[120px] rounded-[10px] px-5 py-[18px] border border-[#FFFFFF1A] shadow-[3px_3px_6px_0px_rgba(0,0,0,0.08)]">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<img class="max-w-[120px] max-h-[50px] opacity-40 inline-block group-hover:opacity-100 group-hover:animate-[hexagonIcon_1s_ease-in-out]" src="<?php echo esc_url($slider[ $slick->get_name_setting('multiple_content_image') ]['url']); ?>" alt="<?php echo esc_attr($slider[ $slick->get_name_setting('multiple_content_title') ]); ?>">
									<?php } ?>
