		...
		<div class="container mx-auto">
			<div class="block_title text-center flex justify-center items-center">
				...
				<?php if ( $widget_title || $widget_sub_title ) { ?>
					<div class="title_block inline-block mb-4">
						<?php
						if ( $widget_sub_title ) {
							?>
					...
					<div class="block_slide">
						...
						foreach ( $sliders as $slider ) {
							...
							<div class="item elementor-repeater-item-<?php echo esc_attr($slider['_id']); ?> basis-0 max-w-full mb-5">
								<div class="block__content relative group duration-300 text-center rounded-[10px] md:px-5 px-2 py-[18px]">
									<?php if ( $slider[ $slick->get_name_setting('multiple_content_image') ]['url'] ) { ?>
										<div class="flex justify-center relative z-10 pt-6 pb-7">
									...
									<?php } ?>
									<h5 class="text-white relative z-10 pb-3">
										<?php
										if ( 'url' === $button_type || $content_link_attr_string ) {
											?>
											<a class="duration-300" <?php echo ! empty($content_link_attr_string) ? $content_link_attr_string : 'href="' . esc_url($button_url) . '"'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
									...
									<?php if ( ! empty($slider[ $slick->get_name_setting('multiple_content_description') ]) ) { ?>
										<div class="relative z-10 text-center text-white pb-3"><?php echo wp_kses_post($slider[ $slick->get_name_setting('multiple_content_description') ]); ?></div>
									<?php } ?>
									...
									if ( $button ) {
										...
										class="rbb-slick-button button relative z-10 mb-8 duration-300 btn_view-all inline-block">
										<span class="text-white duration-300 capitalize hover:text-[color:var(--rbb-general-link-hover-color)]">
											...
											if ( $button_icon ) {
												...
												?>
											<?php } else { ?>
												<i class="rbb-icon-direction-711 text-sm ml-2 align-middle"></i>
											<?php } ?>
										</span>
									</a>
										<?php
