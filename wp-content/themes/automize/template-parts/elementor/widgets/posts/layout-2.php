...
if ( count($posts['posts']) ) {
	?>
	<div id="<?php echo esc_attr($id); ?>" class="rbb_posts <?php echo esc_attr($layout); ?>">
		<div class="2xl:container px-[15px] mx-auto mt-2">
			<div class="block_slide">
				<div class="block_slider__content overflow-hidden">
					<div class="text-center text-white">
						<span class="sub_title mb-4 block"><?php echo wp_kses_post($sub_title); ?></span>
						<h2 class="title text-2xl mb-[38px] block"><?php echo wp_kses_post($posts['title']); ?></h2>
						<?php
						if ( isset($posts['type']) && 'category' === $posts['type'] ) {
							...
							if ( 1 < $cat_count || ( $show_filter && 1 === $cat_count ) ) { 
								?>
								<span class="title-of hidden md:block text-sm uppercase mx-8"><?php echo esc_html__('Of', 'automize'); ?></span>
