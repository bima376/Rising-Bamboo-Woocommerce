...
<main id="primary" class="site-main single-<?php echo esc_attr(get_post_type()); ?> overflow-auto">
	...
	<div class="blog-detail container mx-auto overflow-hidden lg:flex <?php echo esc_attr(( 'on_top' === $position ) ? 'pt-[60px] md:pb-[120px] pb-[60px]' : 'md:pb-[120px] pb-[60px]'); ?>" >
		<?php
		if ( 'left' === $sidebar ) {
			?>
		...
		?>
		<div class="blog-content <?php echo esc_attr(( 'none' !== $sidebar ) ? 'lg:w-[68.23%] md:w-[100%]' : ''); ?> w-full">
			<?php
			while ( have_posts() ) :
				the_post();
				...
				if ( Helper::show_post_navigation() ) {
					the_post_navigation(
						[
							'prev_text' => '<span class="nav-subtitle">' . esc_html__('Previous:', 'automize') . '</span> <span class="nav-title">%title</span>',
							'next_text' => '<span class="nav-subtitle">' . esc_html__('Next:', 'automize') . '</span> <span class="nav-title">%title</span>',
						]
					);
				}
