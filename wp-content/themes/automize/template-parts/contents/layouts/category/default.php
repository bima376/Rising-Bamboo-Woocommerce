	...
	<div class="blog-wrap overflow-hidden p-[15px] rounded-[18px] border duration-300 hover:shadow-[5px_5px_10px_rgba(0,0,0,0.08)]">
		...
		<div class="relative">
				...
				<div class="pt-5 flex font-semibold text-[#909090] text-[10px] uppercase">
					<?php if ( Setting::get(RISING_BAMBOO_KIRKI_FIELD_BLOG_CATEGORY_SHOW_AUTHOR) ) { ?>
						<span class="pr-7 flex items-center">
							...
							<?php } ?>
							<?php echo esc_html__('By ', 'automize'); ?><?php echo get_the_author(); ?>
						</span>
					<?php } ?>
						...
						<span class="comments-link inline-flex">
							...
							if ( $comment_count > 0 ) {
								...
								<?php if ( 1 === $comment_count ) { ?>
									<?php echo esc_attr($comment_count) . '<span>' . esc_html_e('Comment', 'automize') . '</span>'; ?>
								<?php } else { ?>
									<?php echo esc_attr($comment_count) . '<span>' . esc_html__(' Comments', 'automize') . '</span>'; ?>
								<?php } ?>
							<?php } else { ?>
								<?php echo esc_attr($comment_count) . '<span>' . esc_html__(' Comments', 'automize') . '</span>'; ?>
							<?php } ?>
			...
			<div class="entry-content">
				...
				<?php } ?>
				<div class="mt-5 mb-6">
					<a href="<?php the_permalink(); ?>" class="blog-readmore flex items-center text-[#bebebe] hover:text-[color:var(--rbb-general-primary-color)] font-semibold relative mt-6 mb-4"><span class="mr-1.5 relative"><?php echo esc_html__('Read more', 'automize'); ?></span>
						<?php if ( class_exists(RbbIcons::class) ) { ?>
