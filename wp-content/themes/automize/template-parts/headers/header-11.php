...
?>
<header id="rbb-default-header" class="rbb-default-header header-11 z-20 w-full">
	<div class="rbb-header-sticky">
		<div class="header-inner relative mx-auto h-full">
			<div class="header-top mx-auto h-full md:block hidden bg-[#0c1721]">
				<div class="2xl:container px-[15px] relative mx-auto flex flex-nowrap items-center justify-between md:h-[90px] h-[60px]">
					<?php
					if ( Helper::show_logo() ) {
						?>
						<div id="rbb-branding" class="rbb-branding flex py-2.5 items-center rbb-header-left">
					...
					<?php } ?>
					<div class="flex-auto flex items-center justify-center pl-2.5">
					<div class="vertical-menu relative z-50">
						<div class="vertical-menu-title cursor-pointer duration-300 h-[50px] px-5 py-2 lg:flex items-center hidden text-[color:var(--rbb-general-secondary-color)] hover:!text-white bg-black/800 hover:bg-[color:var(--rbb-general-secondary-color)] rounded-[3px] border border-[#242e37]">
							<i class="rbb-icon-menu-6 text-[26px] xl:mr-2.5"></i>
							<span class="text-[10px] uppercase font-semibold xl:block hidden"><?php echo esc_html__('all departments', 'automize'); ?></span>
						</div>
						<div id="header-vertical_menu" class="vertical-menu-content min-w-[270px] absolute top-[60px] left-0 w-full z-10 bg-white rounded-[10px]">
							<?php echo Menu::vertical_menu(); // phpcs:ignore ?>
						</div>
					</div>
					<?php if ( Helper::show_search_product_form() && false === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ) { ?>
						<div id="_desktop_search" class="w-full max-w-[740px] px-2.5">
					...
					if ( Helper::show_login_form() || Helper::show_search_product_form() || Helper::show_mini_cart() || Helper::show_wishlist() ) {
						?>
						<div class="header-right-group flex justify-end items-center">
							<?php if ( Helper::show_login_form() ) { ?>
								<div class="rbb-account relative flex items-center justify-center mr-[25px] <?php echo ( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) ? 'popup-account' : 'toggle-login'; ?> "
									<?php
									if ( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) {
										...
										<div class="pl-1.5 leading-[15px] text-white">
								...
								<?php if ( Helper::show_search_product_form() && Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ) { ?>
									<div id="_desktop_search" class="rbb-header-center flex items-center mr-[25px]">
										<div class="rbb-product-search relative cursor-pointer" onclick="RbbThemeSearch.openSearchForm(event)">
											<div class="rbb-product-search-icon-wrap w-10 h-10 leading-[40px] text-center">
								...
								<?php if ( Helper::show_wishlist() ) { ?>
									<div class="rbb-wishlist flex items-center justify-center mr-[25px]">
										<div class="relative">
											<a class="wishlist-icon-link group duration-300 h-10 w-10 flex items-center justify-center text-center" title="<?php echo esc_attr__('wishlist', 'automize'); ?>" href="<?php echo esc_url(WPcleverWoosw::get_url()); ?>">
								...
								?>
							<div class="vertical-menu relative lg:hidden">
								<div class="vertical-menu-title cursor-pointer duration-300 ml-5">
									<i class="rbb-icon-menu-6 text-[30px] !text-white"></i>
								</div>
							</div>
							</div>
						<?php } ?>
				...
				<div class="bg-white 2xl:container px-[15px] mx-auto overflow-hidden md:block hidden">
				<div class="header-policy flex">	
					<?php
					$sidebar_active = is_active_sidebar('sidebar-header11-top');
					if ( $sidebar_active ) {
						dynamic_sidebar('sidebar-header11-top'); }
					?>
				</div>
				</div>
				<div class="h-full lg:block hidden bg-[color:var(--rbb-header-background-color)]">
					<div class="2xl:container mx-auto">
						<div class="rbb-header-center flex justify-center items-center min-h-[60px]">
				...
				<div class="header-mobile shadow-[0_0px_15px_0_rgba(0,0,0,0.1)] px-[15px] md:hidden">
					<div class="flex items-center justify-between min-h-[60px]">
						<div class="menu-mobile flex items-center min-w-[70px]">
							<span class="toggle-megamenu relative mr-5 cursor-pointer">
								<i class="icon-directional"></i>
							</span>
						...
						<div class="header-mobile-right min-w-[60px] flex items-center justify-end">
							<div id="_mobile_cart" class="rbb-mini-cart justify-end flex"></div>
						</div>
					</div>
				</div>
			</div>
