...
?>
<header id="rbb-default-header" class="rbb-default-header header-5 w-full relative z-20 bg-[color:var(--rbb-header-background-color)]">
	<div class="rbb-header-sticky">
		<div class="header-inner relative mx-auto">
			<div class="container mx-auto md:block hidden overflow-hidden">
				<div class="header-top flex flex-nowrap items-center h-full">
					...
					<div id="rbb-header-top-right" class="rbb-header-right relative h-full ml-3 flex items-center ml-auto mr-0">
						...
						$email          = do_shortcode('[rbb_contact type="email"]');
						$address        = do_shortcode('[rbb_contact type="address"]');
						...
						if ( $sidebar_active ) {
							...
							?>
							<div class="policy py-[35px] md:flex hidden items-center text-white">
								<?php if ( $phone || $email ) { ?>
									<?php
									if ( $email ) {
		...
		<div class="header-center container mx-auto relative z-20">
			<div class="flex flex-nowrap items-center">
				...
				<?php if ( Helper::show_search_product_form() ) { ?>
					<div class="rbb-header-center flex-auto items-center justify-center">
						<?php if ( false === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ) { ?>
						...
						<?php } else { ?>
							<div class="rbb-product-search flex justify-end  cursor-pointer" onclick="RbbThemeSearch.openSearchForm(event)">
								<span class="search-mobile text-center w-[22px] py-3 cursor-pointer text-[22px] text-[color:var(--rbb-search-icon-color)]">
									<i class="rbb-icon-search-10 w-[18px] h-[18px]"></i>
								</span>
								<div class="rbb-product-search-content mb-2.5">
									<?php Tag::search(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY)); // phpcs:ignore ?>
								</div>
							</div>
						<?php } ?>
		...
		<div class="header-bottom h-0 md:block hidden">
			<div class="container mx-auto h-0">
				<div class="lg:w-[285px] w-[65px] relative h-[60px] -top-[60px] vertical-menu pr-[15px] z-[100] <?php echo esc_attr(( is_home() || is_front_page() ) ? 'active' : 'menu-hidden'); ?>">
					<div class="relative">
						<div class="vertical-menu-title text-[color:var(--rbb-general-heading-color)] cursor-pointer bg-white flex items-center lg:rounded-t-[10px] rounded-t-[5px] min-h-[60px] px-[15px]">
							...
							<span class="font-bold uppercase text-xs lg:block hidden"><?php echo esc_html__('Browse All Categories', 'automize'); ?></span>
						</div>
						<div id="header-vertical_menu" class="vertical-menu-content absolute top-[100%] left-0 w-full z-10 border border-[#e2e2e2] shadow-[3px_3px_6px_0px_rgba(0,0,0,0.08)] bg-white rounded-b-[10px] lg:block hidden <?php echo esc_attr(( is_front_page() ) ? 'active' : ''); ?>" >
