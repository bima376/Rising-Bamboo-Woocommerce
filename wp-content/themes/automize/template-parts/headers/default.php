...
?>
<header id="rbb-default-header" class="rbb-default-header header-1 relative z-20 w-full bg-[color:var(--rbb-header-background-color)]">
	<div class="rbb-header-sticky">
		...
		<div class="container mx-auto h-full md:block hidden">
			<div class="rbb-header-center flex pb-2.5">
			<?php if ( has_nav_menu('vertical') ) { ?>
			<div class="vertical-menu relative lg:min-w-[270px] mr-5">
				<div class="vertical-menu-title text-[color:var(--rbb-general-heading-color)] hover:text-white font-bold hover:bg-[color:var(--rbb-general-primary-color)] bg-white lg:px-[18px] px-[13px] cursor-pointer flex items-center rounded-[50px] md:h-[50px] duration-300 group">
					<i class="rbb-icon-menu-6 text-2xl h-[50px] leading-[50px] lg:pr-[14px] lg:mr-4"></i>
					<span class="text-xs lg:inline-block hidden uppercase"><?php echo esc_html__('Browse All Categories', 'automize'); ?></span>
				</div>
				<div id="header-vertical_menu" class="vertical-menu-content min-w-[270px] absolute top-[60px] left-0 w-full z-10 bg-white rounded-b-[10px]  <?php echo ( is_front_page() ) ? 'active' : ''; ?>">
		...
		<?php } ?>
			<?php if ( Helper::show_search_product_form() && false === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ) { ?>
				<div id="_desktop_search" class="flex-auto items-center justify-center">
			...
			if ( $phone ) {
				?>
				<div class="group-contact-phone bg-[color:var(--rbb-general-primary-color)] min-w-[220px] ml-4 rounded-[50px] flex items-center px-[18px] text-white"><a class="!text-white contact-phone"><?php echo do_shortcode('[rbb_contact type="phone"]'); ?></a></div>
			<?php } ?>
		</div>
