			...
			?>
				...
				switch ( $attr_info->type ) {
					case 'color':
						...
						?>
						<li class="rbb-attribute-filter-item item <?php echo esc_attr($active_class); ?> <?php echo esc_attr(( 1 === $show_count ) ? 'show_count block py-2 first:pt-0 last:pb-0 ' : 'inline-block float-left m-1.5'); ?>">
							<a href="<?php echo esc_url($attr_link); ?>" class="filter-link h-[30px]">
								<div class="rounded-full h-[30px] w-[30px] duration-150 border-[1px] block <?php echo esc_attr(( '#ffffff' === $val ) ? ' border-inherit ' : 'border-transparent'); ?> ">
					...
					case 'text':
						?>
						<li class="rbb-attribute-filter-item item m-1.5 inline-block <?php echo esc_attr(( 'inline' === $display ) ? ' float-left' : 'block'); ?> <?php echo esc_attr($active_class); ?> <?php echo esc_attr(( 1 === $show_count ) ? 'show_count' : ''); ?>">
