...
class Woocommerce extends Singleton {
					...
					function ( $tabs = [] ) {
						...
						if ( ! empty($has_related_product) ) {
							$tabs['related_product'] = [
								'title'    => apply_filters('woocommerce_product_related_products_heading', __('Related Products', 'automize')),
								'priority' => 20,
	...
	public function product_get_rating_html( string $html, $rating, $count ): string {
		if ( empty($html) && 0 === (int) $rating ) {
			...
			$label = sprintf(esc_html__('Rated %s out of 5', 'automize'), $rating);
	...
	public function format_review_tab( $tabs ): array {
		...
		if ( isset($tabs['reviews']) ) {
			...
			$tabs['reviews']['title'] = sprintf(__('Reviews <span class="text-xs font-semibold">(%d)</span>', 'automize'), $product->get_review_count());
		}
		return $tabs;
	}
		...
		switch ( $position ) {
			...
			case 'right':
				...
				add_filter(
					'rbb_theme_woocommerce_before_main_content',
					function ( $wrapper_classes ) use ( $position ) {
						if ( $this->show_product_filters() && ( is_product_category() || is_shop() || is_product_tag() || is_tax() ) ) {
	...
	public function product_catalog_layout_setup( array $wrapper_classes ): array {
		...
		if ( is_product_category() || is_shop() || is_product_tag() || is_tax() ) {
	...
	public function render_filter_sidebar(): void {
		...
		if ( $this->show_product_filters() && is_active_sidebar('sidebar-shop-filter') && ( is_product_category() || is_shop() || is_product_tag() || is_tax() ) ) {
			$wrapper_classes = apply_filters(
				'rbb_theme_sidebar_shop_filter',
				[
			...
			<?php if ( 'left' === $position || 'right' === $position || 'top' === $position ) { ?>
				<h2 class="sidebar-filter mt-[100px] text-xl lg:hidden uppercase">
					...
					<?php echo esc_html__('Filter By', 'automize'); ?>
				</h2>
			<?php } ?>
