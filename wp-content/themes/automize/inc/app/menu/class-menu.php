...
class Menu extends Singleton {
	...
	public static function primary_menu( array $args = [] ) {
		...
		if ( class_exists(WalkerNavMenu::class) && ( has_nav_menu('primary') || isset($args['menu']) ) ) {
			...
			echo esc_html__('Please assign a menu to the primary location : Appearance > Menus > Manage Locations', 'automize');
		}
	 ...
	 * @return false|string
	 */
	public static function vertical_menu( array $args = [] ) {
		$class = 'menu-container';
		...
		if ( class_exists(WalkerNavMenu::class) && ( has_nav_menu('vertical') || isset($args['menu']) ) ) {
			$args['walker'] = new WalkerNavMenu();
			wp_nav_menu($args);
		} else {
			echo esc_html__('Please assign a menu to the primary location : Appearance > Menus > Manage Locations', 'automize');
	...
	public static function account_menu( array $args = [] ) {
		...
		if ( class_exists(WalkerNavMenu::class) && has_nav_menu('account') ) {
			...
			echo '<div class="notice whitespace-nowrap">' . esc_html__('Please assign a menu to the account location : Appearance > Menus > Manage Locations', 'automize') . '</div>';
		}
