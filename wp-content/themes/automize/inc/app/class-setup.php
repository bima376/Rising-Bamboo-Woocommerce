...
class ThemeSetup {
	...
	public function __construct() {
		...
		/* 
		 * Trick to fix bug textdomain load early.
		 * If move setup method to the init hook, it will be error when install plguin on setup wizard.
		 */
		add_filter('doing_it_wrong_trigger_error', [ $this, 'prevent_wrong_trigger_load_textdomain' ], 10, 4);
		$this->setup();
	}
	...
	public function setup(): void {
		...
		if ( $theme_config_file ) {
			...
			$trans_default  = [
				'ready-big-button' => esc_html__('View your website', 'automize'),
			];
			if ( class_exists(OCDI_Plugin::class) && class_exists(\RisingBambooCore\App\App::class) && ! $this->setup_complete() ) {
				$trans_default['ready-big-button']      = __('Import demo data', 'automize');
	...
	public function replace_url( $selected_import_index ): void {
		...
		$this->merlin->logger->debug(__('Start replace url', 'automize'));
		if ( isset($import_file_path['content']) ) {
			$path = realpath($import_file_path['content']);
			$this->merlin->logger->info(__('Import file path : ', 'automize'), (array) $path);
			if ( $path ) {
		...
		$this->merlin->logger->info(__('Urls : ', 'automize'), $urls);
		...
		foreach ( $urls as $url ) {
			...
			";
			$this->merlin->logger->info(__('Sql replace : ', 'automize'), [trim($sql)]);
			//phpcs:ignore
			$db_delta = dbDelta($sql);
			$this->merlin->logger->info(__('dbDelta result : ', 'automize'), (array)$db_delta);
			...
			$this->merlin->logger->info(__('dbDelta result : ', 'automize'), (array)$wpdb_result);
			
			$this->merlin->logger->debug(sprintf('End replace a %s with %s', $url, $new_url));
			
			//phpcs:enable
		}
		$this->merlin->logger->debug(__('End replace url', 'automize'));
	}
	...
	public function update_term_count(): void {
		global $wpdb;
		//phpcs:disable
		$this->merlin->logger->debug(__('Update term count', 'automize'));
		...
		//phpcs:enable
		$this->merlin->logger->info(__('Rows affected', 'automize'), [ $result ]);
		$this->merlin->logger->debug(__('End update term count', 'automize'));
	}
	...
	public function update_comment_count(): void {
		//phpcs:disable
		global $wpdb;
		$this->merlin->logger->debug(__('Update comment count', 'automize'));
		...
		foreach ($entries as $entry) {
			...
			$this->merlin->logger->info(__('Update for ID : ', 'automize') . $post_id, [$result]);
		}
		$this->merlin->logger->debug(__('# End update comment count', 'automize'));
		//phpcs:enable
	}
	...
	public function update_woocommerce_pages( $selected_import_index ): void {
		...
		if ( ! empty($selected_import_data) && RisingBambooThemeHelper::woocommerce_activated() && strpos($selected_import_data['import_file_url'], 'extra.xml') !== false ) {
			$this->merlin->logger->debug(__('Update Woocommerce Default Pages', 'automize'));
			$shop = get_posts(
				[
					'post_type'              => 'page',
					'title'                  => 'Shop',
			...
			if ( $shop ) {
				...
				$this->merlin->logger->info(__('Update Shop Page', 'automize'), (array) $shop[0]->ID);
			}
			$cart = get_posts(
				[
					'post_type'              => 'page',
			...
			if ( $cart ) {
				...
				$this->merlin->logger->info(__('Update Cart Page', 'automize'), (array) $cart[0]->ID);
			}
			...
			if ( $checkout ) {
				...
				$this->merlin->logger->info(__('Update Checkout Page', 'automize'), (array) $checkout[0]->ID);
			}
			$account = get_posts(
				[
					'post_type'              => 'page',
			...
			if ( $account ) {
				...
				$this->merlin->logger->info(__('Update Account Page', 'automize'), (array) $account[0]->ID);
			}
		} else {
			$this->merlin->logger->info(__('Not update woocommerce pages', 'automize'), (array) $selected_import_data['import_file_url']);
		}
	}
	...
	public function update_product_meta_lookup(): void {
		if ( RisingBambooThemeHelper::woocommerce_activated() ) {
			$this->merlin->logger->debug(__('Start update product lookup tables', 'automize'));
	...
	public function update_product_attribute_type(): void {
		global $wpdb;
		if ( RisingBambooThemeHelper::woocommerce_activated() ) {
			$this->merlin->logger->debug(__('Update woocommerce product attribute type', 'automize'));
			...
			$this->merlin->logger->info(__('Result : ', 'automize'), [ $sql, $result ]);
			$clear_cache = wp_cache_flush();
			$this->merlin->logger->info(__('Clear all cache : ', 'automize'), [ $clear_cache ]);
			$attribute_transient = delete_transient('wc_attribute_taxonomies');
			$this->merlin->logger->info(__('Delete woocommerce attribute taxonomies transient : ', 'automize'), [ $attribute_transient ]);
			$this->merlin->logger->debug(__('# End update woocommerce product attribute type', 'automize'));
		}
	}
	...
	public function fix_missing_pages(): void {
		if (
			...
			$this->merlin->logger->debug(__('Create missing woocommerce pages', 'automize'));
	...
	public function log_remapped_term( $term_id, $data ): void {
		...
		if ( $original_id && (int) $term_id !== $original_id && isset($data['taxonomy']) ) {
			...
			if ( isset($this->merlin->logger) ) {
				$this->merlin->logger->info(__('Log remapped term', 'automize'), [ $term_id, $original_id ]);
			}
		}
	}

	/**
	 * Prevent _load_textdomain_just_in_time filter notice.
	 *
	 ...
	 * @return bool
	 */
	public function prevent_wrong_trigger_load_textdomain( $trigger, $function_name, $message, $version ): bool {
