...
class Importer extends WXRImporter
{
	...
	protected function get_reader(string $file)
	{
		...
		if (!class_exists('XMLReader')) {
			$this->logger->error(__('The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'automize'), [$file]);
			
			return false;
		}
		
		if (empty($file) || !realpath($file)) {
			$this->logger->error(__('The XML file does not exists!', 'automize'), [$file]);
		...
		if (!$reader) {
			$this->logger->error(__('Could not open the XML file for parsing!', 'automize'), [$file]);
		}
		...
		while ($reader->read()) {
			...
			if ($reader->nodeType == XMLReader::ELEMENT && 'item' === $reader->name) {
				...
				if ($node) {
					...
					// Skip, if there was an error in parsing the item node.
					if (is_wp_error($parsed)) {
						$this->logger->warning(__('Error occur when parsed item node : ' . $node->nodeName, 'automize'));
						$reader->next();
						continue;
					}
					$counter++;
	...
	public function import(string $file, $opts = []): bool
	{
		...
		if (is_wp_error($result)) {
			$this->logger->error(__('Content import start error: ', 'automize') . $result->get_error_message());
	...
	public function new_ajax_request_maybe(array $data): array
	{
		...
		if ($time > apply_filters('rbb-importer/time_for_one_ajax_call', 20)) {
			$response = apply_filters('rbb-importer/new_ajax_request_response_data', [
				'status' => 'newAJAX',
				'log' => __('Time for new AJAX request!: ', 'automize') . $time,
				'num_of_imported_posts' => count($this->mapping['post']),
			]);
			
			// Add message to log file.
			$this->logger->info(__('New AJAX call!', 'automize'));
