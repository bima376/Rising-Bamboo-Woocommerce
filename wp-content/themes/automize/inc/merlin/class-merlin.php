...
class Merlin {
	...
	public function admin_page(): void {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'automize' ),
		);
	...
	public function svg(array $args = array() ): string {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'automize' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'automize' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'automize' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'automize' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'automize' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'automize' ),
	...
	<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'automize' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'automize' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'automize' ) );
	}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'automize' ); ?>">
										<?php esc_html_e( 'req', 'automize' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'automize' ) );
	}
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'automize' ); ?>">
	...
	<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'automize' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The final step has been displayed', 'automize' ) );
	}
	...
	public function generate_child(): void {
        if(check_ajax_referer( 'merlin_nonce', 'wpnonce' )) {
	        ...
	        if ( ! file_exists( $path ) ) {
		        ...
		        $this->logger->debug( __( 'The existing child theme was activated', 'automize' ) );
	        ...
	        $this->logger->debug( __( 'The newly generated child theme was activated', 'automize' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'automize' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'automize' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'automize' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'automize' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'automize' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'automize' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'automize' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'automize' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'automize' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'automize' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'automize' ) );
	...
	public function generate_child_style_css(string $slug, string $parent, string $author, string $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'automize' ) );
	...
	public function generate_child_screenshot( $path ): void
	{
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'automize' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'automize' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	public function _ajax_plugins(): void
	{
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'automize' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'automize' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'automize' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'automize' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'automize' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'automize' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'automize' ),
				)
			);
		}
	...
	public function _ajax_content(): void {
		...
		if ( (empty($_POST['content']) && isset($content[$_POST['content']])) || ! check_ajax_referer( 'merlin_nonce' )) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'automize' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'automize' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'automize' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'automize' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'automize' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items(): void
	{
		if ( empty( $_POST['selected_index'] ) && ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'automize' ) );
			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'automize' ),
				)
			);
		}
	...
	protected function get_import_data(int $selected_import_index = 0 ): array {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'automize' ),
				'description'      => esc_html__( 'Demo content data.', 'automize' ),
				'pending'          => esc_html__( 'Pending', 'automize' ),
				'installing'       => esc_html__( 'Installing', 'automize' ),
				'success'          => esc_html__( 'Success', 'automize' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'automize' ),
				'description'      => esc_html__( 'Sample widgets data.', 'automize' ),
				'pending'          => esc_html__( 'Pending', 'automize' ),
				'installing'       => esc_html__( 'Installing', 'automize' ),
				'success'          => esc_html__( 'Success', 'automize' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'automize' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'automize' ),
				'pending'          => esc_html__( 'Pending', 'automize' ),
				'installing'       => esc_html__( 'Installing', 'automize' ),
				'success'          => esc_html__( 'Success', 'automize' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'automize' ),
				'description'      => esc_html__( 'Sample theme options data.', 'automize' ),
				'pending'          => esc_html__( 'Pending', 'automize' ),
				'installing'       => esc_html__( 'Installing', 'automize' ),
				'success'          => esc_html__( 'Success', 'automize' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'automize' ),
				'description'      => esc_html__( 'Redux framework options.', 'automize' ),
				'pending'          => esc_html__( 'Pending', 'automize' ),
				'installing'       => esc_html__( 'Installing', 'automize' ),
				'success'          => esc_html__( 'Success', 'automize' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'automize' ),
				'description'      => esc_html__( 'After import setup.', 'automize' ),
				'pending'          => esc_html__( 'Pending', 'automize' ),
				'installing'       => esc_html__( 'Installing', 'automize' ),
				'success'          => esc_html__( 'Success', 'automize' ),
	...
	public function import_revolution_sliders(string $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'automize' ) );
	...
	public function rbb_importer_new_ajax_request_response_data( $data ): array {
		...
		$data['message']  = esc_html__( 'Installing', 'automize' );
		$data['proceed']  = 'true';
	...
	public function before_content_import_setup(): void {
		...
		if ( ! empty( $posts ) ) {
			...
			$this->logger->debug( __( 'The Hello world post status was set to draft', 'automize' ) );
		}
	}
	...
	public function validate_import_file_info(array $import_files ): array
	{
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'automize' ), $import_file );
			}
		}
