...
class Tag {
	...
	public static function posted_on(): void {
		...
		/* translators: %s: post date. */
			esc_html_x('Posted on %s', 'post date', 'automize'),
	...
	public static function posted_by(): void {
		...
		/* translators: %s: post author. */
			esc_html_x('by %s', 'post author', 'automize'),
	...
	public static function entry_footer(): void {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list(esc_html__(', ', 'automize'));
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf('<span class="cat-links">' . esc_html__('Posted in %1$s', 'automize') . '</span>', $categories_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list('', esc_html_x(', ', 'list item separator', 'automize'));
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf('<span class="tags-links">' . esc_html__('Tagged %1$s', 'automize') . '</span>', $tags_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					...
					/* translators: %s: post title */
						__('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'automize'),
						[
							'span' => [
								'class' => [],
							],
		...
		edit_post_link(
			sprintf(
				...
				/* translators: %s: Name of current post. Only visible to screen readers */
					__('Edit <span class="screen-reader-text">%s</span>', 'automize'),
					[
						'span' => [
							'class' => [],
						],
	...
	public static function page_title(): void {
		if ( is_category() ) {
			...
			esc_html_e('Posts by " ', 'automize') . the_author() . esc_html_e(' " ', 'automize');
		} elseif ( function_exists('is_shop') && is_shop() ) {
			esc_html_e('Shop', 'automize');
		} elseif ( is_archive() && ! is_search() ) {
			single_term_title();
		} elseif ( is_search() ) {
			// translators: Search for.
			printf(esc_html__('Search for: %s', 'automize'), get_search_query());
		} elseif ( is_404() ) {
			esc_html_e('404 Error', 'automize');
		} elseif ( is_singular('knowledge') ) {
			esc_html_e('Knowledge Base', 'automize');
		} elseif ( is_home() ) {
			esc_html_e('Posts', 'automize');
		} else {
			echo get_the_title();
		}
	}
		...
		$args        = wp_parse_args(
			...
			apply_filters(
				...
				[
					...
					'before'      => '',
					'after'       => '',
					'home'        => _x('Home', 'breadcrumb', 'automize'),
				]
			)
		);
	...
	public static function parse_breadcrumb( string $delimiter_class, string $current_class ): string {
		...
		if ( is_category() ) {
			...
			$result .= wp_kses($before, 'rbb-kses') . esc_html__('Archive by date ', 'automize') . '"' . esc_html(get_the_time('d')) . '"' . $after;
		} elseif ( is_month() ) {
			$result .= '<a href="' . esc_url(get_year_link(get_the_time('Y'))) . '">' . esc_html(get_the_time('Y')) . '</a>' . $delimiter;
			$result .= wp_kses($before, 'rbb-kses') . esc_html__('Archive by month ', 'automize') . '"' . esc_html(get_the_time('F')) . '"' . $after;
		} elseif ( is_year() ) {
			...
			if ( 0 !== $week ) {
				$result .= wp_kses($before, 'rbb-kses') . esc_html__('Archive by year ', 'automize') . '"' . esc_html($week_start_formatted . ' - ' . $week_end_formatted) . '"' . $after;
			} else {
				$result .= wp_kses($before, 'rbb-kses') . esc_html__('Archive by week ', 'automize') . '"' . esc_html($year) . '"' . $after;
			}
			...
			$result .= wp_kses($before, 'rbb-kses') . esc_html__('Search results for : ', 'automize') . '"' . esc_html(get_search_query()) . '"' . $after;
		} elseif ( is_tag() ) {
			$result .= wp_kses($before, 'rbb-kses') . esc_html__('Archive by tag ', 'automize') . '"' . esc_html(single_tag_title('', false)) . '"' . $after;
		} elseif ( is_author() ) {
			global $author;
			$user_data = get_userdata($author);
			$result   .= wp_kses($before, 'rbb-kses') . esc_html__(' Articles posted by ', 'automize') . '"' . esc_html($user_data->display_name) . '"' . $after;
		} elseif ( is_404() ) {
			$result .= wp_kses($before, 'rbb-kses') . esc_html__(' Error 404 not Found ', 'automize') . $after;
			...
			$result .= wp_kses($before, 'rbb-kses') . esc_html__('Blog', 'automize') . $after;
		}
		return $result;
	}
	...
	public static function compare_button( WC_Product $product ): string {
		...
		if ( class_exists('WPCleverWoosc') ) {
			$html .= '<div class="wooscp-compare font-normal" data-tooltips="' . esc_html__('Compare', 'automize') . '">';
	...
	public static function wish_list_button( WC_Product $product ): string {
		...
		if ( RisingBambooCoreHelper::woocommerce_wishlist_activated() ) {
			$html .= '<div class="woosw-wishlist" data-tooltips="' . esc_html__('Wishlist', 'automize') . '">';
	...
	public static function quick_view_button( WC_Product $product ): string {
		...
		if ( Setting::get(RISING_BAMBOO_KIRKI_FIELD_WOOCOMMERCE_QUICK_VIEW_STATUS) ) {
			...
			$html = '<span class="product_quick-view md:flex hidden"><a href="#" data-tooltips="' . esc_html__('Quick View', 'automize') . '" data-product-id="' . esc_attr($product->get_id()) . '" class="quick-view quick-view-button quick-view-' . esc_attr($product->get_id()) . '" >' . $icon . '</a></span>'; //  phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		}
		return wp_kses_post($html);
	}
}
