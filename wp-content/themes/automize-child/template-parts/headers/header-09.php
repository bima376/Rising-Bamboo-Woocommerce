...
?>
<header id="rbb-default-header" class="rbb-default-header header-9 w-full z-20">
	<div class="rbb-header-sticky relative md:block hidden bg-[color:var(--rbb-header-background-color)]">
		<div class="2xl:container mx-auto px-[15px] h-[70px]">
			<div class="header-inner mx-auto h-full">
				<div class="flex flex-nowrap items-center justify-between h-full">
					...
					if ( Helper::show_login_form() || Helper::show_search_product_form() || Helper::show_mini_cart() || Helper::show_wishlist() ) {
						?>
						<div id="rbb-header-right" class="rbb-header-right flex-none h-full flex min-w-[80px] justify-end <?php echo Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY) ? 'border-cart' : ''; ?>">
							...
							<?php } else { ?>
								<div class="rbb-product-search lg:mr-6 mr-4 relative flex items-center justify-center">
									<div id="_desktop_search" class="rbb-product-search-content2 results-full">
										<?php Tag::search(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_OVERLAY)); // phpcs:ignore ?>
									</div>
									<div class="rbb-product-search-icon-wrap cursor-pointer hidden w-8 h-[50px] justify-center items-center flex">
										<span class="rbb-product-search-icon duration-300 <?php echo esc_attr(Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_SEARCH_ICON)); ?>"></span>
									</div>
								</div>
							<?php } ?>
							<?php if ( Helper::show_login_form() ) { ?>
								<div class="rbb-account relative flex items-center justify-center mr-5
								<?php echo ( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) ? 'popup-account' : 'toggle-login'; ?> "
									...
									if ( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) {
										...
										>
									<div class="rbb-account-icon-wrap relative duration-300 cursor-pointer bg-white h-10 w-[50px] flex items-center justify-center">
							...
							<?php if ( Helper::show_wishlist() ) { ?>
								<div class="rbb-wishlist flex items-center justify-center mr-4">
									<div class="relative">
	...
	<div class="header-mobile shadow-[0_0px_15px_0_rgba(0,0,0,0.1)] relative bg-[color:var(--rbb-header-background-color)] px-[15px] md:hidden">
