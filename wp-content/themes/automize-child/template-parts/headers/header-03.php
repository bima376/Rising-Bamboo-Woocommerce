...
?>
<header id="rbb-default-header" class="rbb-default-header header-3 w-full md:h-[110px] relative z-[2000]">
	<div class="container header-top hidden md:!grid grid-cols-5 mx-auto h-[65px]">
		...
		<div class="col-span-2 flex justify-self-end items-center">
			...
			if ( Helper::show_login_form() || Helper::show_wishlist() ) {
				...
				<div class="rbb-account min-h-[65px] relative md:flex hidden items-center justify-center mr-3.5
					<?php echo esc_attr(( false === is_user_logged_in() && true === Setting::get(RISING_BAMBOO_KIRKI_FIELD_COMPONENT_ACCOUNT_POPUP) ) ? 'popup-account' : 'toggle-login'); ?> "
								<?php
	...
	<div class="rbb-header-sticky relative md:absolute top-[65px] inset-0 z-[999] md:h-[90px] md:block hidden">
		<div class="container mx-auto h-full md:px-[15px] px-5">
					...
					if ( Helper::show_logo() ) {
						...
						<div id="rbb-branding" class="rbb-branding xl:col-span-1 lg:col-span-3 md:col-span-5 flex items-center rbb-header-left">
							<div class="vertical-menu relative">
								<div class="vertical-menu-title relative justify-center bg-[#eeeeee] ml-5 mr-10 my-5 cursor-pointer flex items-center rounded md:w-[50px] md:h-[50px] duration-300">
									<i class="rbb-icon-menu-6 text-2xl"></i>
								</div>
								<div id="header-vertical_menu" class="vertical-menu-content min-w-[270px] absolute top-[100%] left-0 w-full z-10  bg-white rounded-b-[10px] lg:block hidden">
